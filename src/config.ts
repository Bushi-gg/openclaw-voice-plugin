...
export const SttConfigSchema = z
  .object({
    /** STT provider ("openai" for OpenAI Whisper, "groq" for Groq Whisper) */
    provider: z.enum(["openai", "groq"]).default("openai"),
    /** Whisper model to use */
    model: z.string().min(1).default("whisper-1"),
    /** API key for the selected STT provider (falls back to env vars) */
    apiKey: z.string().min(1).optional(),
    /** Base URL override for the STT API */
...
export const VoiceCallStreamingConfigSchema = z
  .object({
    ...
    sttProvider: z.enum(["openai-realtime", "groq"]).default("openai-realtime"),
    /** OpenAI API key for Realtime API (uses OPENAI_API_KEY env if not set) */
    openaiApiKey: z.string().min(1).optional(),
    /** Groq API key for Whisper STT (uses GROQ_API_KEY env if not set) */
  ...
  .default({
    enabled: false,
    sttProvider: "openai-realtime" as const,
