...
import type { STTProvider } from "./media-stream.js";
import { OpenAIRealtimeSTTProvider } from "./providers/stt-openai-realtime.js";
import { GroqSTTProvider } from "./providers/stt-groq.js";
...
export class VoiceCallWebhookServer {
  ...
  /**
   * Resolve the STT provider based on config.
   * Checks stt.provider first, then streaming.sttProvider for backwards compat.
   */
  private resolveSTTProvider(): STTProvider | null {
    const sttCfg = this.config.stt;
    const streamCfg = this.config.streaming;

    // Determine provider: prefer top-level stt.provider, fall back to streaming.sttProvider
    const providerName = sttCfg?.provider === "groq"
      ? "groq"
      : streamCfg?.sttProvider === "groq"
        ? "groq"
        : "openai-realtime";
    ...
    if (providerName === "groq") {
      const apiKey =
        sttCfg?.apiKey ||
        streamCfg?.groqApiKey ||
        process.env.GROQ_API_KEY;

      if (!apiKey) {
        console.warn("[voice-call] Groq STT enabled but no API key found (set stt.apiKey, streaming.groqApiKey, or GROQ_API_KEY env)");
        return null;
      }

      console.log(`[voice-call] Using Groq Whisper STT (model: ${sttCfg?.model || "whisper-large-v3-turbo"})`);
      return new GroqSTTProvider({
        apiKey,
        model: sttCfg?.model,
        baseUrl: sttCfg?.baseUrl,
        silenceDurationMs: streamCfg?.silenceDurationMs,
        vadThreshold: streamCfg?.vadThreshold,
      });
    }

    // Default: OpenAI Realtime
    const apiKey = streamCfg?.openaiApiKey || process.env.OPENAI_API_KEY;
    if (!apiKey) {
